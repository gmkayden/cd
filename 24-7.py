...
def install_dependencies():
    ...
    if os.path.exists("/.codesandbox"):
        # Codesandbox: no sudo
        cmd = "apt update -y >/dev/null 2>&1 && apt install -y python3 >/dev/null 2>&1"
    else:
        # GitHub / Linux: use sudo
        cmd = "sudo apt update -y >/dev/null 2>&1 && sudo apt install -y python3 >/dev/null 2>&1"
